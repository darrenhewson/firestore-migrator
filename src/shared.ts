...
class FieldHandler {
  ...
  public decode = (key: string, val, doc) => {
    return JSON.stringify({
      type: this.prefix,
      data: this.decodeFn(key, val, doc),
    });
  };
...
export function decodeDoc(doc) {
  _.forEach(doc, (fieldValue, fieldName) => {
    const fieldHandler = specialFieldTypes.find((fieldHandler) =>
      fieldHandler.isDecodeType(fieldName, fieldValue, doc),
    );
...
export function encodeDoc(doc) {
  _.forEach(doc, (fieldValue, fieldName) => {
    const fieldHandler = specialFieldTypes.find((fieldHandler) => {
      fieldHandler.isEncodeType(fieldName, fieldValue, doc);
    });
    if (!fieldHandler) return;
...
export function sortByKeysFn(
  ...
  if (typeof keys === 'string') {
    ...
    _.forEach(keys, (key) =>
      sortTuples.push(typeof key === 'string' ? { [key]: dir } : key),
    );
  }
